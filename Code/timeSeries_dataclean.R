library(forecast)
setwd('/Users/ADITYA/Desktop/Datathon/git-repo/duke-datathon-team18/Data/')
...
# function for creating CSV file for a City in the State
# Data: City, Real Estate, GDP, Per Capita, Unmployment Rate, Employment Rate
# Years in rows: 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016
datacity <- function(city, state){
  cityData <- read.csv(paste(state, "csv", sep = '.'), header = TRUE)
...
datastate <- function(state){
  ...
  write.csv(df, file = paste(state, "csv", sep = '.'), row.names = TRUE)  
  
}

# Creating CSV files for the States
for (i in unique(real_estate$state)){
    datastate(i)
}

# Creating City files for the Particular State
